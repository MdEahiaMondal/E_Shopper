                    ...
                    <div class="brands-name">
                        <ul class="nav nav-pills nav-stacked">
                            ...
                            @foreach($categories as $category)
                                    <li><a class="{{ (request()->is('category-product/'.$category->slug)) ? 'active':''}}" href="{{URL:: to('category-product/'.$category->slug)}}"> <span class="pull-right">( {{ $count = count(\App\Product::where(['category_id'=>$category->id, 'deleted_at'=>null])->get()) }} )</span>{{$category->name}}</a></li>
                            @endforeach
                ...
                <div class="brands-name">
                    <ul class="nav nav-pills nav-stacked">
                        ...
                        @foreach($brands as $brand)
                            <li><a class="{{ (request()->is('brand-product/'.$brand->slug)) ? 'active':''}}" href="{{URL:: to('brand-product/'.$brand->slug)}}"> <span class="pull-right">( {{ $count = count(\App\Product::where(['brand_id' =>$brand->id, 'deleted_at'=>null])->get()) }} )</span>{{$brand->name}}</a></li>
                        @endforeach
